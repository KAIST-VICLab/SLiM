...
from slim.eval.utils import ModelWithIntermediateLayers, evaluate
...
def get_args_parser(
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Number of action classes (60 for NTU-60, 120 for NTU-120)",
    )
    parser.add_argument(
        "--benchmark",
        type=str,
    ...
    parser.add_argument(
        "--eval-only",
        action="store_true",
        help="Skip training and evaluate the trained classifiers given by --classifier-fpath",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
    ...
    parser.set_defaults(
        ...
        epochs=10,  # maybe longer for NTU
        batch_size=16,  # 128
        num_workers=8,
        epoch_length=2000,  # None
        save_checkpoint_frequency=20,
        eval_period_iterations=2000,  # None
...
def load_linear_classifiers(state_dict, sample_output, num_classes):
    """Rebuild a trained classifier grid from its state dict.

    Classifier names encode their own architecture (`classifier_<n>_blocks_avgpool_<bool>_lr_...`),
    so a released grid loads regardless of the world size it was trained with — unlike
    `setup_linear_classifiers`, whose names depend on the LR scaling rule.
    """
    linear_classifiers_dict = nn.ModuleDict()
    for name in sorted({k.split(".")[1] for k in state_dict}):
        fields = name.split("_")
        ...
        linear_classifiers_dict[name] = LinearClassifier(
            out_dim, use_n_blocks=n_blocks, use_avgpool=avgpool, num_classes=num_classes
        ).cuda()

    linear_classifiers = AllClassifiers(linear_classifiers_dict)
    linear_classifiers.load_state_dict(state_dict, strict=True)
    logger.info(f"Loaded {len(linear_classifiers_dict)} trained linear classifiers")
...
def test_on_datasets(
    ...
    for test_dataset_str, class_mapping, metric_type in zip(test_dataset_strs, test_class_mappings, test_metric_types):
        ...
        test_data_loader = make_eval_data_loader(test_dataset_str, batch_size, num_workers, metric_type, data_path, num_classes, benchmark)
...
def run_eval_linear(
    ...
    if eval_only:
        if not classifier_fpath:
            raise ValueError("--eval-only requires --classifier-fpath")
        state = torch.load(classifier_fpath, map_location="cpu")
        linear_classifiers = load_linear_classifiers(
            state["model"] if "model" in state else state, sample_output, training_num_classes
        )
        val_data_loader = make_eval_data_loader(val_dataset_str, batch_size, num_workers,
                                                val_metric_type, data_path, num_classes, benchmark)
        ...
        best = results["best_classifier"]
        logger.info(f"Accuracy: {100.0 * best['accuracy']:.2f} ({best['name']})")
        return {"best_classifier": best["name"], f"{val_dataset_str}_accuracy": 100.0 * best["accuracy"]}
    ...
    val_data_loader = make_eval_data_loader(val_dataset_str, batch_size, num_workers, val_metric_type, data_path, num_classes, benchmark)
...
if __name__ == "__main__":
    description = "SLiM linear evaluation"
